...
namespace ElectronicShopManagement.Forms
{
    public partial class Sell : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select Cashier name.", "Warning",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            var minusPro = Products_Stock.SharedProducts
                .FirstOrDefault(p => p.ProductName == comboboxsellproname.Text);

            if (minusPro == null)
            {
                MessageBox.Show("Please select a valid product.", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            int qty = Convert.ToInt32(txtsellqty.Text);
            if (minusPro.StockQuantity < qty)
            {
        ...
        private async Task CheckPaymentAsync(string md5)
        {
            ...
            while (DateTime.Now - startTime < timeout)
            {
                ...
                {
                    MessageBox.Show("Payment Successful!", "Bakong KHQR",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);

                    //SalesRepository.AddSaleRange(products);
                    var productReport = products.Select(r => new SalesRepository
                    {
                        Name = r.Name,
                        Prices = r.Prices,
                        SellQty = r.SellQty,
                    ...
                    SalesRepository.report.AddRange(productReport);
                    ...
                    var recentData = products.Select(p => new RecentSell
                    {
                        Name = p.Name,
                        Prices = p.Prices,
                        SellQty = p.SellQty,
        ...
        private async void btnpayment_Click(object sender, EventArgs e)
        {
            ...
            if (totalAmount <= 0)
            {
                MessageBox.Show("No items to sell.", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            foreach (Form f in Application.OpenForms)
            {
                if (f is BakongKhqr)
                {
                    MessageBox.Show("Payment not completed within 1 minute.", "Timeout",
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    f.Close();
                    break;
                }
        ...
        {
            string selectedCategory = comboboxsell.SelectedItem.ToString();
            var productsList = Products_Stock.SharedProducts
                .Where(p => p.Category == selectedCategory)
                .ToList();

            comboboxsellproname.DisplayMember = "ProductName";
            comboboxsellproname.ValueMember = "ProductID";
            comboboxsellproname.DataSource = productsList;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show("Are You sure for cancel?", "Info",
                MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            var selectedProduct = tblshowproductsell.CurrentRow.DataBoundItem as ProductForSellModel;
            if (selectedProduct != null)
            {
                var stockItem = Products_Stock.SharedProducts
                    .FirstOrDefault(p => p.ProductName == selectedProduct.Name);

                if (stockItem != null)
                    stockItem.StockQuantity += selectedProduct.SellQty;
